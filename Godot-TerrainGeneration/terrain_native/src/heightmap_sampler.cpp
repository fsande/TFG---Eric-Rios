...
void HeightmapSamplerNative::_bind_methods() {
  ClassDB::bind_method(D_METHOD("bake_heightmap", "heightmap"),
    &HeightmapSamplerNative::bake_heightmap);
  ClassDB::bind_method(D_METHOD("generate_height_grid",
    "chunk_bounds", "resolution", "terrain_size", "height_scale"),
    &HeightmapSamplerNative::generate_height_grid);
  ClassDB::bind_method(D_METHOD("is_baked"),
    &HeightmapSamplerNative::is_baked);
}
...
void HeightmapSamplerNative::bake_heightmap(const Ref<Image>& heightmap) {
  ...
  // Read all pixels once. Image::get_pixel is not thread-safe to call
  // concurrently, so we bake here on the main thread and read in parallel later.
...
PackedFloat32Array HeightmapSamplerNative::generate_height_grid(
  AABB chunk_bounds,
  int resolution,
  float terrain_size,
  float height_scale
