 ...
 * @brief Declares a native (C++) heightmap sampler used by terrain generation.
 */
 ...
 *
 * This is a drop-in replacement for the GDScript height grid hot loop.
 *
 * ## Thread-safety
 * - Call bake_heightmap() once (typically on the main thread) to copy data out of
 *   a Godot Image.
...
public:
   ...
   *
   * The resulting array is a regular grid in XZ over @p chunk_bounds.
   * Values are bilinearly interpolated from the baked heightmap and scaled by
   * @p height_scale.
   *
   ...
   *
   * @return PackedFloat32Array containing the sampled heights. If the sampler
   *     is not baked, an empty array is returned and an error is pushed.
   */
  PackedFloat32Array generate_height_grid(
    AABB chunk_bounds,
    int resolution,
    float terrain_size,
    float height_scale
...
private:
  ...
  /// Baked heightmap width in pixels.
  int _width  = 0;

  /// Baked heightmap height in pixels.
  int _height = 0;
