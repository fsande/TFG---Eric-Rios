...
#func test_construction_with_valid_data():
	#test_mesh_data = MeshData.create(test_vertices, test_indices, test_uvs)
	#assert_not_null(test_mesh_data, "Should create valid MeshData")
...
#func test_get_height_returns_y_component():
	...
	#test_mesh_data = MeshData.create(vertices, PackedInt32Array(), uvs)
...
#func test_mismatched_vertex_uv_count_is_handled():
	...
	#test_mesh_data = MeshData.create(vertices, indices, uvs)
...
#func test_height_values_are_correctly_extracted():
	...
	#var uvs := PackedVector2Array([
		...
		#Vector2(0, 1), Vector2(1, 1)
	#])
	#test_mesh_data = MeshData.create(vertices_with_heights, PackedInt32Array(), uvs)
