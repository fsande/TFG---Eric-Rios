...
func is_pair_valid(
	...
	# If more than threshold is underwater, probably different landmasses
	var is_valid := underwater_ratio <= max_underwater_ratio
	
	if not is_valid:
		print("HeuristicValidator: REJECTED pair (%.1f%% underwater, threshold %.1f%%)" % [
			underwater_ratio * 100.0,
			max_underwater_ratio * 100.0
		])
