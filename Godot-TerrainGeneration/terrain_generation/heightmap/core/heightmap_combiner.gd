...
func combine_cpu(_images: Array[Image], _context: ProcessingContext) -> Image:
	push_error("combine_cpu() must be implemented by subclass")
	return null
...
func get_combiner_name() -> String:
	push_error("get_combiner_name() must be implemented by subclass")
	return "Unknown Combiner"
