...
@tool
class_name GdscriptCpuChunkGenerationStrategy extends ChunkGenerationStrategy
...
func _apply_subtractive_volume(
	...
	if new_indices.size() < mesh_data.indices.size():
		var result := MeshData.create(mesh_data.vertices, new_indices, mesh_data.uvs)
...
func _apply_additive_volume(
	...
	var result := MeshData.create(new_vertices, new_indices, new_uvs)
	result.width = mesh_data.width
	result.height = mesh_data.height
	result.mesh_size = mesh_data.mesh_size
	return result
