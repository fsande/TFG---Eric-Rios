...
func _init(terrain_def: TerrainDefinition, base_resolution: int = 64) -> void:
...
func generate_chunk_at_bounds(chunk_bounds: AABB, lod_level: int = 0) -> ChunkMeshData:
	var chunk_size := Vector2(chunk_bounds.size.x, chunk_bounds.size.z)
	var coord := _bounds_to_coord(chunk_bounds, chunk_size)
	return generate_chunk(coord, chunk_size, lod_level)
...
func _bounds_to_coord(bounds: AABB, chunk_size: Vector2) -> Vector2i:
	var terrain_size := _terrain_definition.terrain_size
	var half_terrain := terrain_size / 2.0
	var x := int(floor((bounds.position.x + half_terrain.x) / chunk_size.x))
	var z := int(floor((bounds.position.z + half_terrain.y) / chunk_size.y))
	return Vector2i(x, z)
